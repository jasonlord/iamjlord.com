...
function injectCSS() {
  ...
  if (scriptSrc) {
    const cssUrl = scriptSrc.replace('/fintech-dashboard.js', '/fintech-dashboard.css');
...
function initFintechDashboard() {
  ...
  function animate() {
    ...
    if (progress < 0.35) {
      ...
      labelDesktop.className = "text-xs font-medium px-2 py-1 rounded bg-neutral-900 text-white transition-all duration-300";
      labelTablet.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      labelMobile.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      ...
      labelDesktop.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      labelTablet.className = "text-xs font-medium px-2 py-1 rounded bg-neutral-900 text-white transition-all duration-300";
      labelMobile.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      ...
      labelDesktop.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      labelTablet.className = "text-xs font-medium px-2 py-1 rounded text-neutral-400 transition-all duration-300";
      labelMobile.className = "text-xs font-medium px-2 py-1 rounded bg-neutral-900 text-white transition-all duration-300";
    }
    ...
    // Apply transforms
    if (desktopOp > 0) {
      desktop.style.transform = `perspective(1000px) scale(${desktopScale}) rotateX(${currentRotateX}deg) rotateY(${currentRotateY}deg)`;
    }
    if (tabletOp > 0) {
      tablet.style.transform = `perspective(1000px) scale(${tabletScale}) rotateX(${currentRotateX}deg) rotateY(${currentRotateY}deg)`;
    }
    if (mobileOp > 0) {
      mobile.style.transform = `perspective(1000px) scale(${Math.min(1, mobileScale)}) rotateX(${currentRotateX}deg) rotateY(${currentRotateY}deg)`;
    }
